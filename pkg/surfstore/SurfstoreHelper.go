...
func checkDeletedFile(hashBlocks []string) bool {
	if len(hashBlocks) != 1 {
		return false
	}
	if hashBlocks[0] == "0" {
...
func FileMetaDataToStringNew(fm *FileUpdateInfo) (result string) {
	result += fm.fileName + ","
	result += strconv.Itoa(int(fm.version)) + ","
...
func WriteMetaFileNew(fileMetas map[string]*FileUpdateInfo, baseDir string) error {
	outputMetaPath := filepath.Clean(ConcatPath(baseDir, DEFAULT_META_FILENAME))
	...
	for _, fileMeta := range fileMetas {
		_, err := outFD.WriteString(FileMetaDataToStringNew(fileMeta))
		if err != nil {
			log.Fatal("Error During Meta Write Back")
		}
	}
